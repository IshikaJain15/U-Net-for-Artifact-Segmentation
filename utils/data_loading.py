...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = '_label1'):
        ...
        self.missing_masks = []  # List to store names of images without masks

        # Check for missing masks during initialization
        for file in listdir(images_dir):
            if isfile(join(images_dir, file)) and not file.startswith('.'):
                img_id = splitext(file)[0]
                mask_file = list(self.mask_dir.glob(img_id + mask_suffix + '.*'))
                if not mask_file:
                    self.missing_masks.append(img_id)  # Add to list if mask not found

        # Log missing masks
        if self.missing_masks:
            logging.warning(f'Masks not found for the following images: {self.missing_masks}')
        ...
        with Pool() as p:
            unique = list(tqdm(p.imap(partial(unique_mask_values, mask_dir=self.mask_dir, mask_suffix=self.mask_suffix), self.ids),
            total=len(self.ids)))
...
class MilanDataset(BasicDataset):
    def __init__(self, images_dir, mask_dir, scale=1):
        super().__init__(images_dir, mask_dir, scale, mask_suffix='_label1')
